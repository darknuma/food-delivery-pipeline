    ...
    def _write_parquet_to_s3(self, topic):
        ...
        try:
            ...
            table = pa.Table.from_pandas(
                pd.DataFrame(self.buffer[topic]), schema=schema
            )
            ...
            self.s3_client.put_object(
                Bucket=self.bucket, Key=s3_key, Body=buf.getvalue()
            )
            logger.info(
                f"Successfully wrote {len(self.buffer[topic])} records to s3://{self.bucket}/{s3_key}"
            )
...
if __name__ == "__main__":
    ...
    topics = os.getenv(
        "KAFKA_TOPICS",
        "food-delivery-orders-raw,food-delivery-merchants-raw,food-delivery-reviews-raw",
    ).split(",")
