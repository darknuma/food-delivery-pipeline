...
from pyspark.sql.functions import (
    col, avg, count, sum, when, datediff, 
    hour, date_format, abs, round, current_timestamp
)
import os
import logging
...
def calculate_fulfillment_metrics(orders_df):
    ...
    return orders_df.groupBy("event_date").agg(
        ...
        round(count(when(col("order_status") == "DELIVERED", True)) / count("order_id") * 100, 2).alias("fulfillment_rate"),
        count(when(col("order_status") == "CANCELLED", True)).alias("cancelled_orders"),
        round(count(when(col("order_status") == "CANCELLED", True)) / count("order_id") * 100, 2).alias("cancellation_rate"),
        round(avg(when(col("is_delayed") == True, 1).otherwise(0)) * 100, 2).alias("delay_rate")
    )

def calculate_delivery_time_metrics(orders_df):
    """Calculate delivery time metrics"""
    return orders_df.filter(col("order_status") == "DELIVERED").groupBy("event_date").agg(
        round(avg("delivery_time_minutes"), 2).alias("avg_delivery_time"),
        round(avg(when(col("is_delayed") == True, col("delivery_time_minutes"))), 2).alias("avg_delayed_delivery_time"),
        round(avg(when(col("is_delayed") == False, col("delivery_time_minutes"))), 2).alias("avg_ontime_delivery_time")
    )
...
def calculate_order_volume_metrics(orders_df, order_items_df):
    ...
    order_metrics = orders_df.join(order_items_df, "order_id", "left").groupBy("event_date").agg(
    ...
    hourly_distribution = orders_df.groupBy(
        "event_date",
        hour("event_timestamp").alias("hour")
    ).count().groupBy("event_date").agg(
...
def calculate_eta_accuracy_metrics(orders_df):
    ...
    return orders_df.filter(col("order_status") == "DELIVERED").groupBy("event_date").agg(
        round(avg(abs(col("delivery_time_minutes") - 
                    col("estimated_delivery_time").cast("timestamp"))), 2).alias("avg_eta_deviation"),
