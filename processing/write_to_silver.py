...
def explode_order_items(df):    
    df = df.select("order_id", F.explode("items").alias("item")) \
        .select(
            "order_id", 
            "item.item_id", 
            "item.name", 
            F.col("item.quantity").cast(IntegerType()).alias("quantity"),  # Cast 'quantity' to Integer
            F.col("item.unit_price").cast(DoubleType()).alias("unit_price"),  # Cast 'unit_price' to Float
            F.col("item.total_price").cast(DoubleType()).alias("total_price")  # Cast 'total_price' to Float
        )
