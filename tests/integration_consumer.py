...
def kafka_s3_messages():
    return [
        MagicMock(
            value=json.dumps({"event_id": "123", "merchant_id": "merchant_1"}).encode(
                "utf-8"
            )
        )
    ]
...
def test_kafka_to_cassandra(
    mock_kafka_consumer, mock_cassandra_session, kafka_messages
):
    mock_kafka_consumer.return_value.poll.return_value = {
        "food-delivery-orders-raw": kafka_messages
    }

    consumer = KafkaToCassandraConsumer(
        "localhost:9092", "test-group", ["localhost"], "test_keyspace"
    )
...
def test_kafka_to_s3(mock_kafka_consumer, mock_s3_client, kafka_s3_messages):
    mock_kafka_consumer.return_value.poll.return_value = {
        "food-delivery-orders-raw": kafka_s3_messages
    }
